    ...
    for yyyy_mm in sorted((demanda_map or {}).keys()):
        ...
        if usar_escalonado and len(fechas) > 0:
            ...
            for dia, cantidad in conteo_por_dia.items():
                # Modo de pago configurable: 'siempre_dos' o 'solitario'
                try:
                    modo = str(getattr(cfg, 'modo_pago_ayudantes', 'siempre_dos') or 'siempre_dos').lower()
                except Exception:
                    modo = 'siempre_dos'
                pago = 0
                # Determinar número de ayudantes según modo
                if modo == 'solitario' and cantidad == 1:
                    # Paga a 1 ayudante con escala solitario si existe; si no, usa general
                    num_ayudantes_dia = 1
                    try:
                    ...
                    fuente = escalas_solitario_sorted if len(escalas_solitario_sorted) > 0 else escalas_sorted
                else:
                    # Siempre paga a numero_ayudantes usando escala general
                    try:
                        num_ayudantes_dia = int(getattr(cfg, 'numero_ayudantes', 2))
                    except Exception:
                        num_ayudantes_dia = 2
                    fuente = escalas_sorted
                for umbral, monto in fuente:
                    if cantidad >= umbral:
                        pago = monto
                    else:
                        break
                if pago == 0 and fuente:
                    pago = fuente[0][1]
