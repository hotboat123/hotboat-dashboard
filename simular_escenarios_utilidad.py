...
def resumen_operacion_mensual(
    ...
    - CVOps: suma de df_cost_op EXCLUYENDO filas con descripcion == 'pago extra ayudante (diario)'
    - CTrab: suma de df_cost_op SOLO donde descripcion == 'pago extra ayudante (diario)'
    - costos_fijos: suma df_fijos por mes
    - utilidad: ingresos - (CVOps + CTrab + costos_fijos)
    """
    ...
    # separar costo de ayudantes vs operativos
    if df_cost_op is None or df_cost_op.empty:
        g_ctrab = pd.DataFrame(columns=["mes", "CTrab"]).assign(CTrab=pd.Series(dtype=float))
        g_cvops = pd.DataFrame(columns=["mes", "CVOps"]).assign(CVOps=pd.Series(dtype=float))
    else:
        dco = df_cost_op.copy()
        dco["mes"] = pd.to_datetime(dco["fecha"]).dt.to_period("M").astype(str)
        is_trab = dco["descripcion"].astype(str).str.lower() == "pago extra ayudante (diario)"
        df_trab = dco[is_trab]
        df_ops = dco[~is_trab]
...
def ejecutar_escenario(nombre: str, factor: float) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    try:
        ...
        if opm.empty:
            ...
            g_cvops = float(opm["CVOps"].sum()) if "CVOps" in opm.columns else 0.0
            g_ctrab = float(opm["CTrab"].sum()) if "CTrab" in opm.columns else 0.0
